...
const YourComponent: React.FC = () => {
  ...
  const [feedDto, setFeedDto] = useState<FeedDto>({
    userId: "",
    content: "",
    visibility: "1",
    tags: [],
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isChecked) {
        ...
        formData.append(
          "feedDto",
          new Blob([JSON.stringify(newFeedDto)], { type: "application/json" })
        );
        if (img) {
          formData.append("img", img, img.name); // 이미지 파일과 파일 이름을 함께 전달
        }
      ...
      if (isChecked2) {
        ...
        formData.append(
          "feedDto",
          new Blob([JSON.stringify(newFeedDto2)], { type: "application/json" })
        );
        if (img) {
          formData.append("img", img, img.name); // 이미지 파일과 파일 이름을 함께 전달
        }
              ...
              <div className={style.contentDiv}>
                ...
                <div className={style.select}>
                  <select className={style.selectClub} onChange={onChangeClub}>
                    ...
                    {exerciseTitle && (
                      <option value="1">{exerciseTitle}</option>
                    )}
                  ...
                  <div className={style.tag}>
                    ...
                    <input
                      ...
                      style={{ border: "none", fontSize: "15px", width: "75%" }}
                    />
                  </div>
                </div>
              </div>
