...
export default function getPostRecommend() {
  ...
  const { detailFeedId, canChange, setCanChange } = homeStore();
  ...
  const fetchFeedData = async () => {
    try {
      const response = await FeedService.recommendFeed(userId); //피드 불러오기
  ...
  const handleScrollToComponent = (id: number) => {
    console.log(canChange);
    console.log(detailFeedId);
    const element = document.getElementById(id.toString());
    if (element) {
      element.scrollIntoView({ behavior: "smooth", block: "end" });
      setCanChange(false, 0);
    }
  };
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (canChange) {
        handleScrollToComponent(detailFeedId);
      }
    }
  }, [ref.current]);
  ...
  return (
    <div className={style.postContainer}>
      {feeds &&
        feeds.map((feed) => (
          <div className={style.post} id={feed.feedId.toString()} ref={ref}>
            <div className={style.profile}>
              <img
                className={style.profileImage}
                src={feed.userImgUrl}
                onMouseOver={() => {
                  setPointFeed(feed.feedId);
                }}
                onMouseOut={() => {
                  setPointFeed("0");
                }}
              ...
              {feed.feedId === pointFeed && (
                <div
                  ...
                  className={style.profileImageCover}
                  onClick={() => {
